...
# This script fits A/f^B to the power spectra using initial guesses from "new-fit-x.dat" within the fit range "modified-range-x.dat."  The fit parameters as a function of temperature are written to "refit-x.dat."
...
i1=1
previousfit=read.table("new-fit-x.dat",header=FALSE)
previousrange=read.table("modified-range-x.dat",header=FALSE)
...
for (k in seq(from=i1,to=i2,by=1))
{
	...
	sink('refit-x.dat',append=T)
		cat(temperature$T[k+pr-1],"\t",Axn,"\t",Bxn,"\n",sep="")
	sink()
	Agxp=Agx
	Bgxp=Bgx

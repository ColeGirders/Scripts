...
for (k in setdiff(i1:i2,skip))
{
	...
	else
	{
		...
		while(Ld3==0)
		{
			name=sprintf("Data/FFT/ffttn-10-%d.txt",k)
			datar=read.table(name,header=FALSE, col.names=c("fd","xd"))
			datarx=datar[,1:2]
			L=length(datarx[,1])
			x1=datarx[1,1]
			y1=datarx[1,2]
...
# A/f^B is fit over equal ranges on a log-log plot.  The percent change (error1) between the previous (Bp) and current exponents (Bn) are calculated.  If the 1/f region, the noise exponent (B) should be relatively constant, so the percent change should be smaller
			...
			for (j in seq(from=1,to=((Pf-Pi)*Ld-N+2),by=1)){
				...
				sink('range-error.dat',append=TRUE)
				if (Ln>2){
	...
	sink('original-range.dat',append=TRUE)
	...
	fftr=read.table(sprintf("Data/FFT/ffttn-10-%d.txt",k),header=FALSE,col.names=c("fd","xd"))
...
# If the frequency range is too small, it is set to be the frequency range of the previous temperature.
	...
	sink('new-range.dat',append=TRUE)
		cat(temperature$T[k],"\t",k,"\t",r1n,"\t",r2n,"\n",sep="")
	sink()

	sink('modified-range.dat',append=TRUE)
		cat(temperature$T[k],"\t",k,"\t",r1n,"\t",r2n,"\n",sep="")
	sink()
	...
	Bgy=-(log10(y2n)-log10(y1n))/(log10(f2n)-log10(f1n))
	Agy=10**(log10(y2n)+Bgy*log10(f2n))
	Bgz=-(log10(z2n)-log10(z1n))/(log10(f2n)-log10(f1n))
	Agz=10**(log10(z2n)+Bgz*log10(f2n))
	...
	sink('new-fit.dat',append=T)
		cat(temperature$T[k],"\t",(Axn+Ayn+Azn)/3.0,"\t",(Bxn+Byn+Bzn)/3.0,"\n",sep="")
	sink()
	Agxp=Agx
	Bgxp=Bgx
	Agyp=Agy
